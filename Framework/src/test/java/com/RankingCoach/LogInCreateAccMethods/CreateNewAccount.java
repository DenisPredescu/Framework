...
public class CreateNewAccount extends TestEnvironment {
	...
	public void CreateAccountLogInWithGoogle() throws InterruptedException, IOException {
		...
		driver.switchTo().window(childpage);
		Thread.sleep(1000);
		action.moveToElement(el.GoogleEmail()).sendKeys("loremipsum2109@gmail.com").perform();
		action.sendKeys(Keys.ENTER).perform();
		Thread.sleep(1000);
		action.moveToElement(el.GooglePass()).sendKeys("test2109");
		Thread.sleep(2000);
		action.sendKeys(Keys.ENTER).perform();
		...
		if (googleerror.equals(prop.getProperty("googleerror"))) {
			...
			login.login().click();
			Thread.sleep(1000);
			action.click(login.username()).sendKeys("loremipsum2109@gmail.com").perform();
			Thread.sleep(1000);
			action.click(login.password()).sendKeys("test2109").perform();
			Thread.sleep(1000);
			action.click(login.submit()).perform();
